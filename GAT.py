...
def get_data(data_source, variable, grid, year, month, days, frequency, statistic, extent):
    ...
    data = ct.catalogue.retrieve(data_source,
        {
...
def get_data_all(data_source, variable, grid, start, end, frequency, statistic, extent):
    months = pandas.date_range(start = start.replace(day = 1), end = end, freq = 'MS')
    if len(months) == 1:
        days = [day for day in range(start.day, end.day + 1)]
        data = get_data(data_source, variable, grid, months[0].year, months[0].month, days, frequency, statistic, extent)
    if len(months) >= 2:
        daysStart = [day for day in range(start.day, calendar.monthrange(months[0].year, months[0].month)[1] + 1)]
        dataStart = get_data(data_source, variable, grid, months[0].year, months[0].month, daysStart, frequency, statistic, extent)
        if len(months) >= 3:
            for month in months[1 : -1]:
                daysMid = [day for day in range(1, calendar.monthrange(month.year, month.month)[1] + 1)]
                dataMid = get_data(data_source, variable, grid, month.year, month.month, daysMid, frequency, statistic, extent)
                dataStart = ct.cube.concat([dataStart, dataMid], dim = 'time')
        daysEnd = [day for day in range(1, end.day + 1)]
        dataEnd = get_data(data_source, variable, grid, months[-1].year, months[-1].month, daysEnd, frequency, statistic, extent)
...
def process_apparent_temperature(dataAT, unit, city, start):
    ...
    fig1 = ct.map.plot(dataATMonth, title = 'Apparent temperature (' + str(start)[0: 7] + ')')
...
def apparent_temperature(data_source, start, end, method, frequency, statistic, grid, unit, extent, city):
    if method == 'Humidex TD':
        dataTemp = get_data_all(data_source, '2m_temperature', grid, start, end, frequency, statistic, extent)
        dataTemp = ct.cdm.convert_units(dataTemp, 'Celsius')
        dataDewp = get_data_all(data_source, '2m_dewpoint_temperature', grid, start, end, frequency, statistic, extent)
        ...
        dataTemp = get_data_all(data_source, '2m_temperature', grid, start, end, frequency, statistic, extent)
        dataTemp = ct.cdm.convert_units(dataTemp, 'Celsius')
        dataWindU = get_data_all(data_source, '10m_u_component_of_wind', grid, start, end, frequency, statistic, extent)
        dataWindV = get_data_all(data_source, '10m_v_component_of_wind', grid, start, end, frequency, statistic, extent)
        dataDewp = get_data_all(data_source, '2m_dewpoint_temperature', grid, start, end, frequency, statistic, extent)
        ...
        dataDewp = get_data_all(data_source, '2m_dewpoint_temperature', grid, start, end, frequency, statistic, extent)
        dataDewp = ct.cdm.convert_units(dataDewp, 'Celsius')
        dataRadi = get_data_all(data_source, 'surface_net_solar_radiation', grid, start, end, frequency, statistic, extent)
        dataHumi = (dataDewp + 19.2 - 0.84 * dataTemp) / (0.1980 + 0.0017 * dataTemp)
        dataPres = dataHumi / 100 * 6.105 * pow(np.e, 17.27 * dataTemp / (237.7 + dataTemp))
        dataWind = pow(dataWindU * dataWindU + dataWindV * dataWindV, 0.5)
        dataAT = dataTemp + 0.348 * dataPres - 0.7 * dataWind + 0.7 * (dataRadi / (3600 * 3)) / (dataWind + 10) - 4.25
        dataAT = ct.cdm.update_attributes(dataAT, attrs = {'units': 'Celsius'})
        dataATMonth, fig1, fig2 = process_apparent_temperature(dataAT, unit, city, start)
        return dataAT, dataATMonth, fig1, fig2
...
layout.add_widget(row = 0, content = 'data_source')
...
@ct.input.dropdown('data_source', label = 'Data source', values = ['reanalysis-era5-land', 'reanalysis-era5-single-levels-preliminary-back-extension', 'reanalysis-era5-single-levels'])
@ct.input.calendar('start', label = 'Start date', default = '2020-06-01', date_range = ['1950', '2022'], format = 'day')
@ct.input.calendar('end', label = 'End date', default = '2020-08-31', date_range = ['1950', '2022'], format = 'day')
@ct.input.dropdown('grid', label = 'Grid', values = [0.1, 0.25, 0.5, 1])
...
def calculator(data_source, start, end, method, frequency, statistic, grid, unit, extent, city):
    start = datetime.datetime.strptime(start, '%Y-%m-%d')
    end = datetime.datetime.strptime(end, '%Y-%m-%d')    
    dataAT, dataATMonth, fig1, fig2 = apparent_temperature(data_source, start, end, method, frequency, statistic, grid, unit, extent, city)
    return dataAT, dataATMonth, fig1, fig2